...
public class BowlingPage2 extends JFrame {

	private JPanel contentPane;
	private static int bowlingId = 0;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					BowlingPage2 frame = new BowlingPage2(bowlingId);
	 ...
	 * Create the frame.
	 */
	public BowlingPage2(int bowlingId) {
		BowlingPage2.bowlingId = bowlingId;
		...
		btnNewButton_1_3_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				dispose();
				BowlingPlacePage2 cpp;
				try {
					cpp = new BowlingPlacePage2();
					cpp.setVisible(true);
		...
		JLabel lblNewLabel = new JLabel("BOWLING INFORMATION");
		...
		JLabel availabilityLabel = new JLabel(String.valueOf(BowlingPlace.bowlings.get(1).isAvailability()));
		...
		JLabel prıceLabel = new JLabel(String.valueOf(BowlingPlace.bowlings.get(bowlingId).getPrice()));
		...
		JLabel totalPrıceLabel = new JLabel(
				String.valueOf(BowlingPlace.bowlings.get(bowlingId).getPrice() * (int) spinner.getValue()));
		...
		confirmButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (checkEnoughCredit(Integer.parseInt(totalPrıceLabel.getText()))) {
					...
					BowlingPlace.bowlings.get(bowlingId).rent((int) spinner.getValue() * 60);
